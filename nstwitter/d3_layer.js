function d3layer(layername, config){
		...
		this.coolcircles = config.coolcircles;
		...
		_this.onmousclick = function(item,data){
			...
			var starttime = new Date(now - (3600 * 1000) ).toISOString();
			
			var url = "http://research.geodan.nl/service/treinen/location/api/trail?username=ns&trackeeName="+data.properties.id+"&startTime="+starttime+"&endTime="+endtime;
			d3.json(url,function(arr){
